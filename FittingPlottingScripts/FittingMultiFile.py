# -*- coding: utf-8 -*-
"""
Created on Mon Aug  5 13:50:37 2024
...
if __name__ == '__main__':
    ...
    ###for automatically processing multiple files in given directory
    ...
    dir_list = [f for f in os.listdir(my_dir) if '.data.h5' in f]
    
    #for loop of files to be processed in the filequeue
    for f in dir_list:
        data_filepath = './filequeue/' + f
    ...
    #Insert fraction=0.046, pad=0.04 to scale the colorbars appropriately
        ...
        plt.savefig('intenvelwidthplot/'+ gauss_cutout1.meta['mod_index']['date_obs'][11:19].replace(':',''), dpi = 300)
    ...
    #plotting spectra FEXXIII
        ...
        profile_subplt.legend(loc='upper center', frameon=False)
    ...
    # print(fit_x_vals)
        ...
        plt.savefig('intenvelwidthspecplot/'+ gauss_cutout1.meta['mod_index']['date_obs'][11:19].replace(':',''), dpi = 300)
    ...
    # ####    
        ...
        profile_subplt2.legend(loc='upper center', frameon= False)
        plt.ylabel('Residual Intensity \n [erg / (s sr $cm^2$)]')
        ...
        profile_subplt3.legend(loc='upper center', frameon=False)
        ...
        profile_subplt4.legend(loc='upper center', frameon = False)
        profile_subplt4.set_xlabel('Wavelength [$\AA$]')
        #define limits for residual axis
        # plt.ylim(-4000,3300)
        plt.xlim(262.68, 264)
    
        plt.savefig('spectraplots/'+ gauss_cutout1.meta['mod_index']['date_obs'][11:19].replace(':',''), dpi = 300) 
        
        ######################################
        ####Plotting for multispectral plot####
        #######################################
        ...
        plt.savefig('multispectraplots/'+ gauss_cutout1.meta['mod_index']['date_obs'][11:19].replace(':',''), dpi = 200)
        ######################################
        ...
        plt.savefig('intenplotsnocbar/'+ gauss_cutout1.meta['mod_index']['date_obs'][11:19].replace(':',''), dpi = 300)
        ...
        flat_vel1 = np.ravel(vel_map1.data)
        flat_vel2 = np.ravel(vel_map2.data)
        flat_width1 = np.ravel(width_map1.data)
        flat_width2 = np.ravel(width_map2.data)
        
        #function to provide the vel vals for the filtered kappa values
         
        def vel_filt(flatvel, kapfilt):
            velcontain = []
            for i in range(len(flatvel)):
                if np.ravel(kapfilt)[i] <= 20:
                    velcontain.append(flatvel[i])
            #Ensures that if no corresponding chi2 vals are found for good kappas, container becomes 0 so np.mean() works with it
            #NOTE: WHEN GOING THROUGH CHI2 FOR GOOD VALS, EXCLUDE 0 CHI2 VALUES WHEN AVERAGING!!!!
            if velcontain == []:
                velcontain = 0
        ...
        #function to provide the width vals for the filtered kappa values 
        
        def width_filt(flatwidth, kapfilt):
            widthcontain = []
            for i in range(len(flatwidth)):
                if np.ravel(kapfilt)[i] <= 20:
                    widthcontain.append(flatwidth[i])
            #Ensures that if no corresponding chi2 vals are found for good kappas, container becomes 0 so np.mean() works with it
            #NOTE: WHEN GOING THROUGH CHI2 FOR GOOD VALS, EXCLUDE 0 CHI2 VALUES WHEN AVERAGING!!!!
            if widthcontain == []:
                widthcontain = 0
        ...
        #produce absolute values for doppler velocities 
        goodvel1 = np.absolute(vel_filt(flat_vel1, kap_filt1))
        goodvel2 = np.absolute(vel_filt(flat_vel2, kap_filt2))
        
        #goodkappa widths
        goodwidth1 = width_filt(flat_width1, kap_filt1)
        goodwidth2 = width_filt(flat_width2, kap_filt2)
        ...
        #goodvalue array recording number of good kappas for Fe16, Fe23, Mean Kappas for Fe16, Fe23, Std. Devs.Fe16, Fe23 , Mean Chi2 for Fe16, Fe23, Std. Devs for Chi2 for Fe16, Fe23, and the recorded observation time
        #additional inclusion of velocities and widths, post observation time
        GoodVals = np.array([[goodkapct1, goodkapct2, np.mean(goodkapval1), np.std(goodkapval1), np.mean(goodkapval2), np.std(goodkapval2),
                              np.mean(goodchi1), np.std(goodchi1), np.mean(goodchi2), np.std(goodchi2), gauss_cutout1.meta['mod_index']['date_obs'][11:19],
                              np.mean(goodvel1), np.std(goodvel1), np.mean(goodvel2), np.std(goodvel2), np.mean(goodwidth1), np.std(goodwidth1),np.mean(goodwidth2), 
                              np.std(goodwidth2)]])
        ...
        plt.savefig('kfiltplotsnocbar/'+gauss_cutout1.meta['mod_index']['date_obs'][11:19].replace(':',''), dpi = 300)
    ...
    ####CREATE KAPPA INTENSITY OVERLAY####
        ...
        plt.savefig('koverlayplotsnocbar/'+gauss_cutout1.meta['mod_index']['date_obs'][11:19].replace(':',''), dpi = 300)
