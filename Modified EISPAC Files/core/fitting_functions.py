__all__ = ['multigaussian', 'multigaussian_deviates', 'multiKappa', 'multiKappa_deviates']

import sys
import numpy as np


#importing datatest3... .py file to provide necessary kappa input
# from datatest3fullfitFE16FE23 import kappavalue
# import sys
 
## adding datatest3... .py to path
# sys.path.insert(0, '\Owner\Documents\Course Material\MSc\Project\CatalogTest\data_eis\datatest3fullfitFE16FE23.py')

# #assigning kappavalue to kappa from the main script file
...
def multigaussian(param, x, n_gauss=1, n_poly=0):
    ...
    for n in range(n_gauss):
        p = param[3*n:3*n+3]
        arg = ((x-p[1])/p[2])**2  #original gaussian arg
        f = f + p[0]*np.exp(-arg/2.0) #original gaussian function outpt
...
# -----------------------------------------------------------------
# MODIFICATIONS for fitting Kappa Functions 
# -----------------------------------------------------------------

# series of Kappa-like model functions with polynomial background
def multiKappa(param, x, n_gauss=1, n_poly=0):
    """Multigaussian model with a polynomial background
    ...
    for n in range(n_gauss):
        ...
        # arg = ((x-p[1])/p[2])**2  #original gaussian arg
        #arg = (1 + ((x - p[1])**2)/(2 * (p[2]**2) * kappa)) #original kappa arg
        
        #σ2=σ2_κ / (1−3/2κ)  ---> σ2_κ = σ2(1−3/2κ)  indicates 'variance'-like term which is required for implementation
        newvar = p[2] * (1 - 3/(2*p[4]))
        arg = (1 + ((x - p[1])**2)/(2 * (newvar**2) * p[4]))
        # f = f + p[0]*np.exp(-arg/2.0) #original gaussian function outpt
        f = f + p[0] * (arg) ** (-1 *p[4])
...
# computes deviates between fit model and data
def multiKappa_deviates(param, x=None, y=None, error=None,
                           n_gauss=None, n_poly=None, fjac=None, debug=False):
    """Computes the deviates between a multiKappa model fit and input data.
    ...
    # compute model function
    model = multiKappa(param, x, n_gauss, n_poly)
