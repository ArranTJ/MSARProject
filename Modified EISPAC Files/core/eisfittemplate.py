...
class EISFitTemplate:
    ...
    def __init__(self, filename=None, template=None, parinfo=None, **kwargs):
        ...
        if isinstance(parinfo, (list, tuple, dict)):
            ...
            print('Error: Invalid datatype for "parinfo". Please input a '
                 ...
                 +'parameters.', file=sys.stderr)
        # #testing print statements to deduced data structures?    
        # print("parinfo?:", parinfo)
        # print("parinfo index?:", parinfo[0]['fixed'])
        # print("parinfo index type?", type(parinfo[0]['fixed']))
        ...
        if ('n_gauss' not in self.template.keys()) or ('n_poly' not in self.template.keys()) or ('kappaval' not in self.template.keys()):
            if len(self.parinfo) > 0:
                ...
                if 'kappaval' not in self.template.keys():
                    self.template['kappaval'] = int(10)
            else:
        ...
        if len(self.parinfo) == 0:
            ...
            kappaval = max(1, self.template.get('kappaval', 1))

            par_list = []
            for g in range(3*n_gauss):
                par_list.append({'value': 1.0,
                                  'limited': np.array([1, 0], dtype='int16')})
            for k in range(kappaval):
                par_list.append({'value': 10.0,
                                  'limited': np.array([1, 0], dtype='int16')})              
    ...
    def _validate_parinfo_list(self):
        ...
        n_params = len(self.parinfo)
        print("length of params", n_params)
        for p in range(n_params):
            # First, ensure all keys are lower case
    ...
    def __repr__(self):
        ...
        rows.append(f"kappavalue: {self.template['kappaval']}")
        rows.append('')
